...
NDiplomacy = {
	...
	BASE_BOOST_PARTY_POPULARITY_DAILY_PP = 100, 						-- Daily pp cost for boost party popularity
	BASE_BOOST_PARTY_POPULARITY_DAILY_DRIFT = 0.00, 						-- Daily amount of popularity that will be added by the activity.
	BASE_STAGE_COUP_DAILY_PP = 100,										-- Daily pp cost for staging a coup
	...
	TENSION_VOLUNTEER_FORCE_DIVISION = 0.05,							-- Amount of tension generated for each sent division
	...
	IDEOLOGY_JOIN_FACTION_MIN_LEVEL = 0.2,			-- ideology limit required to join faction
	...
	LICENSE_ACCEPTANCE_OPINION_FACTOR = 0.3,		-- Opinion modifier for acceptance of license production requests.
	LICENSE_ACCEPTANCE_PUPPET_BASE = 30,			-- Acceptance modifier for puppets requesting production licenses.
	LICENSE_ACCEPTANCE_TECH_DIFFERENCE = 10, 		-- Acceptance modifier for each year of technology difference.
	LICENSE_ACCEPTANCE_TECH_DIFFERENCE_BASE = 5,    -- Acceptance base for tech difference
	LICENSE_ACCEPTANCE_SAME_FACTION = 50,			-- Acceptance modifier for being in the same faction
},
...
NCountry = {
	...
	POPULATION_YEARLY_GROWTH_BASE = 0.015,				-- was 0.015 basic population growth per year, used for monthly manpower gain
	...
	AIR_SCORE_MULTIPLIER = 0.02,							-- Based on number of planes (which is typically a lot).
	...
	RESOURCE_IMPORTANCE_FACTOR = 0.25,					-- State resource importance factor for AI and calculations
	...
	MIN_NAVAL_SUPPLY_EFFICIENCY = 0.45,					-- Min ratio when supplies will be considered delivered from the capital by naval path
	...
	STATE_VALUE_NON_CORE_STATE_FRACTION = 0.40,			-- If a state is not a core we assume we will get 50% of the factory slots
...
NProduction = {
	...
	INFRA_MAX_CONSTRUCTION_COST_EFFECT = 0.4, 				-- Building in a state with higher infrastructure will reduce the cost of shared buildings.
...
NBuildings = {
	...
	NAVALBASE_REPAIR_MULT = 0.400,					-- Each level of navalbase building repairs X strength. The value is spread on all ships needed reparation. Fe Navalbase lvl 5 x 0.5 str repair = 2.5 str repaired over 10 ships, so each ship repair hourly 0.25 str.
	...
	SABOTAGE_FACTORY_DAMAGE = 35.0,					-- How much damage takes a factory building in sabotage when state is occupied. Damage is mult by (1 + resistance strength), i.e. up to 2 x base value.
...
NMilitary = {
	COMBAT_VALUE_ORG_IMPORTANCE = 1.1,						-- Multiplier on TotalOrganisation when determining the combat value of a division
	COMBAT_VALUE_STR_IMPORTANCE = 1.2,						-- Multiplier on TotalStrength when determining the combat value of a division
	
	HOURLY_ORG_MOVEMENT_IMPACT = -0.15,						-- how much org is lost every hour while moving an army.
	...
	MAX_ARMY_EXPERIENCE = 8000,								--Max army experience a country can store
	...
	BASE_FORT_PENALTY = -0.35, 					   			-- fort penalty
	...
	SUPPLY_USE_FACTOR_INACTIVE = 0.5,
	SUPPLY_GRACE = 70,										-- troops always carry 3 days of food and supply
	MAX_OUT_OF_SUPPLY_DAYS = 30, 				   			-- how many days of shitty supply until max penalty achieved
	OUT_OF_SUPPLY_ATTRITION = 0.4,                 			-- max attrition when out of supply
	...
	PLAN_MIN_AUTOMATED_EMPTY_POCKET_SIZE = 3,				-- The battle plan system will only automatically attack provinces in pockets that has no resistance and are no bigger than these many provinces
	PLAN_SPREAD_ATTACK_WEIGHT = 50.0,						-- DNM, original 13 (AI overhaul mod) -- The higher the value, the less it should crowd provinces with multiple attacks.
	PLAN_SUPPLY_FACTOR = 10,								-- DNM, original 0.1 (AI mod) -- Factor applied to available supply on a front location when determining priority
	...
	PLAN_PROVINCE_BASE_IMPORTANCE = 1.0,					-- 1.0 Used when calculating the calue of front and defense area provinces for the battle plan system
	...
	PLAN_PROVINCE_HIGH_VP_IMPORTANCE_FRONT = 4.0,   		-- DNM, original 4.0  (AI overhaul mod) -- Used when calculating the calue of fronts in the battle plan system
	...
	PLAN_PORVINCE_PORT_LEVEL_FACTOR = 0.55,					-- Bonus factor for port level
	...
	PLAN_PROVINCE_PRIO_DISTRIBUTION_MIN = 0.75,		-- Lowest fraction of divisions that will be distributed based on province priority
	...
	PLAN_BLITZ_OPTIMISM = 0.1,								-- Additional combat balance value in favor of blitzing side when considering targets (not a combat bonus, just offsets planning)
	...
	COMBAT_STACKING_START = 10,								-- at what nr of divisions stacking penalty starts
...
NAir = {
	...
	DAY_NIGHT_COVERAGE_FACTOR = 0.45,					-- How much of the land in the region must be covered by night to consider doing night missions. The same with the day.
	...
	AGGRESSION_THRESHOLD = { 0.2, 0.3, 0.5 },			-- Threshold levels for mission aggressivity for air
	...
	SUPPLY_NEED_FACTOR = 0.35, 							-- multiplies supply usage
	CAPACITY_PENALTY = 1.2,								-- scales penalty of having overcrowded bases.
...
NNavy = {
	...
	COMBAT_MIN_WIN_CHANCE_TO_ADVANCE_WITHOUT_WAIT = 0.85, 				-- compare our forces with enemy forces. 1.0 is equal win chances. Less then 1.0 is we have low chances. If we have at least X chances, then ships will attept to attack without waiting for the other ships that are on the way.
	COMBAT_MIN_WIN_CHANCE_TO_KEEP_FIGHTING = 0.65,		 				-- DNM - original 0.2 - being at 60 per cent of the enemy's strength would usually lead to one side disengaging, and seems to work reasonably in tests -- compare our forces with enemy forces. 1.0 is equal win chances. Less then 1.0 is we have low chances. If we have at least X chances, then ships will attept to attack without waiting for the other ships that are on the way.
	...
	COMBAT_TORPEDO_ATTACK_MAX_RANGE = 4.1,								-- DNM, consistent with vanilla changes, better for naval combat -- max range for torpedo attack
	COMBAT_TORPEDO_ATTACK_USE_CHANCE = 0.26,							-- DNM, 0.5 is too high, vessels make more torpedo attacks than they would have been able too historically -- chance of using torpedo attack (to avoid torpedo spam when at close distance)
	...
	COMBAT_EVASION_TO_HIT_CHANCE = 0.009,								-- BICE, we take ship evasion stats, and mult by this value, so it gives hit chance reduction. So if reduction is 0.025 and ship evasion = 10, then there will be 0.25 (25%) lower hit chance. (Fe. 50% base -25% from evasion +10% bcoz it's very close).
	...
	MAX_EVASION_BONUS = 0.3,											-- largest evasion penalty to hitting
	TARGETING_STACKING_HIT_CHANCE_PENALTY = 0.08,                    	-- penalty to hit chance for each ship targeting same target
	COMBAT_LOW_ORG_HIT_CHANCE_PENALTY = -0.75,							-- % of penalty applied to hit chance when ORG is very low.
	...
	COMBAT_TORPEDO_CRITICAL_DAMAGE_MULT = 3.0,							-- BICE, multiplier to damage when got critical hit from torpedo. (Critical hits are devastating as usualy torpedo_attack are pretty high base values).
	...
	COMBAT_SHIP_SPEED_TO_FIELD_FACTOR = 0.6,							-- Modifier to the ships speed in the combat. For better balance, make it lower to slow down the speed of approaching.
	...
	COMBAT_DETECTED_CONVOYS_FROM_SURFACE_DETECTION_STAT = 0.5,			--# Each 1.0 of surface_detection that ship has (equipment stat), gives x% of convoys discovered from total travelling along the route.
	...
	REPAIR_AND_RETURN_AMOUNT_SHIPS_LOW = 0.33,							-- % of total damaged ships, that will be sent for repair-and-return in one call.
	REPAIR_AND_RETURN_AMOUNT_SHIPS_MEDIUM = 0.5,						-- % of total damaged ships, that will be sent for repair-and-return in one call.
	...
	REPAIR_AND_RETURN_UNIT_DYING_STR = 0.3,								-- Str below this point is considering a single ship "dying", and a high priority to send to repair.
	EXPERIENCE_LOSS_FACTOR = 1.00,                 						-- percentage of experienced solders who die when manpower is removed
	...
	ANTI_AIR_ATTACK_TO_AMOUNT = 0.02,									-- BICE, original 0.05 -- Balancing value to convert equipment stat anti_air_attack to the random % value of airplanes being hit.
	...
	NAVAL_TRANSFER_BASE_SPEED = 5,                                 		-- base speed of units on water being transported
	...
	SEA_AIR_COMBAT_MAX_WINGS_ON_STACK =  220,                     		-- DNM original 200 -- how many attacker planes can enter naval combat per wave
...
NTrade = {
	DISTANCE_TRADE_FACTOR = -0.02,			-- DNM, original -0.02 (AI mod) -- Trade factor is modified by distance times this
	RELATION_TRADE_FACTOR = 1,					-- Trade factor is modified by Opinion value times this
	FEAR_TRADE_FACTOR = 1,
	ALLOW_TRADE_CUT_OFF = 5,					-- If trade factor is less than this, no trade will be allowed
	...
	BASE_TRADE_FACTOR = 85,						-- This is the base trade factor
...
NAI = {
	GARRISON_FRACTION = 0, 							-- DNM, vanilla is 0, and apparently this screws with the AI (shuffling troops to and from fronts) -- How large part of a front should always be holding the line rather than advancing at the enemy
	...
	DIPLOMACY_LEAVE_FACTION_OPINION_FACTOR = 0.60,		-- How much the opinion of countries being targeted by faction leader matters when deciding to leave a faction (conservatism only)
	...
	DIPLOMACY_ACCEPT_VOLUNTEERS_BASE = 25,				-- Base value of volunteer acceptance (help is welcome)
	...
	RESEARCH_LAND_DOCTRINE_NEED_GAIN_FACTOR = 0.18, 	-- Multiplies value based on relative military industry size / country size.
	RESEARCH_NAVAL_DOCTRINE_NEED_GAIN_FACTOR = 1.65, 	-- Multiplies value based on relative naval industry size / country size.
	RESEARCH_AIR_DOCTRINE_NEED_GAIN_FACTOR = 1.65, 		-- Multiplies value based on relative number of air base / country size.
	...
	RESEARCH_BONUS_FACTOR = 3.5, 						-- To which extent AI should care about bonuses to research
	...
	RESEARCH_BASE_DAYS = 30,							-- AI adds a base number of days when weighting completion time for techs to ensure it doesn't only research quick techs
	DECLARE_WAR_RELATIVE_FORCE_FACTOR = 0.5,			-- Weight of relative force between nations that consider going to war
	TRADEABLE_FACTORIES_FRACTION = 0.55,					-- DNM, original 0.3 (AI Mod) -- Will at most trade away this fraction of factories.
	...
	NAVAL_BASES_SHIP_FACTOR = 1.1,						-- BICE -- The extent to which number of naval bases play into amount of sips a nation wants
	NAVAL_STATES_SHIP_FACTOR = 0.75,						-- BICE -- The extent to which number of states play into amount of sips a nation wants
	...
	DIVISION_UPGRADE_MIN_XP = 550,						-- Minimum XP before attempting to upgrade a division template.
	...
	CALL_ALLY_FASCIST_DESIRE = -30,						-- Desire to call ally added for fascist AI
	CALL_ALLY_COMMUNIST_DESIRE = 75,					-- Desire to call ally added for communist AI
	JOIN_ALLY_BASE_DESIRE = 10,							-- exactly what it says
	JOIN_ALLY_DEMOCRATIC_DESIRE = 50,					-- Desire to join ally added for conservatism AI
	...
	LENDLEASE_FRACTION_OF_PRODUCTION = 0.3,				-- Base fraction AI would send as lendlease
	...
	NEIGHBOUR_WAR_THREAT_FACTOR = 1.50, 				-- How much increase in threat does AI feel against neighbours who are at war
	...
	RELATIVE_STRENGTH_TO_INVADE = 0.25,					-- Compares the estimated strength of the country/faction compared to it's enemies to see if it should invade or stay at home to defend.
	RELATIVE_STRENGTH_TO_INVADE_DEFENSIVE = 0.7,		-- Compares the estimated strength of the country/faction compared to it's enemies to see if it should invade or stay at home to defend, but while being a defensive country.
	...
	EASY_TARGET_FRONT_IMPORTANCE = 10,					-- How much extra focus the AI should put on who it considers to be the easiest target.
	...
	FRONT_REASSIGN_DISTANCE = 120.0,					-- DNM, original 100.0 (ai overhaul mod) -- If a unit is this far away from a front it is not considered to be assigned to it unless the new front is much more important
	...
	ENTRENCHMENT_WEIGHT = 4.0,				    		-- DNM, original 100.0 (ai overhaul mod) -- AI should favour units with less entrenchment when assigning units around.
	...
	UNIT_ASSIGNMENT_TERRAIN_IMPORTANCE = 20.0,			-- DNM, original 10 -- Terrain score for units are multiplied by this when the AI is deciding which front they should be assigned to
	...
	DIVISION_DESIGN_COMBAT_WIDTH_WEIGHT = -2.5,				-- This score is reduced the higher width is when comparing pure changes with no target
	...
	BUILD_ARMOR_BASE_COST_WEIGHT = 0.0,
	...
	REGION_THREAT_PER_SUNK_CONVOY = 40,						-- Threat value per convoy sunk in a region. Decays over time.
	...
	MAX_DISTANCE_NALAV_INVASION = 150.0,					-- DNM, changed from 500 (AI Mod) -- AI is extremely unwilling to plan naval invasions above this naval distance limit.
	...
	PRODUCTION_MAX_PROGRESS_TTO_SWITCH_NAVAL = 0.5,		-- AI will not replace ships being built by newer types if progress is above this
	STATE_CONTROL_FOR_AREA_DEFENSE = 0.15,					-- To avoid AI sending area defense to area with very little foothold
	...
	NAVAL_MISSION_PATROL_NEAR_OWNED = 26000,				-- Extra patrol mission score near owned provinces
	...
	MIN_UNITS_FACTOR_AREA_ORDER = 0.25,						-- DNM, original 0.5 (AI mod) -- Factor for min number of units to assign to area defense orders

	MAX_UNITS_FACTOR_FRONT_ORDER = 5.0,						-- DNM, original 1 (AI mod) -- Factor for max number of units to assign to area front orders
	DESIRED_UNITS_FACTOR_FRONT_ORDER = 5.5,					-- DNM, original 1 (AI mod) -- Factor for desired number of units to assign to area front orders
	...
	DESIRED_UNITS_FACTOR_INVASION_ORDER = 0.5,				-- Factor for desired number of units to assign to naval invasion orders
	...
	AIR_SUPERIORITY_MISSION_FACTOR = 3.0,					-- AI air superiority mission factor
	CAS_MISSION_FACTOR = 1.2,								-- AI cas mission factor
	INTERCEPTION_MISSION_FACTOR = 2.5,						-- AI interception mission factor
	STREATEGIC_BOMBER_MISSION_FACTOR = 1.5,					-- AI strategic bombing mission factor
	NAVAL_BOMBER_MISSION_FACTOR = 1.0,						-- AI naval bombing mission factor
	NUKE_MISSION_FACTOR = 2.0,								-- AI nuke mission factor
	PARADROP_MISSION_FACTOR = 1.0,							-- AI paradrop mission factor
	KAMIKAZE_MISSION_FACTOR = 0.5,							-- AI naval kamikaze mission factor
	PORT_STRIKE_MISSION_FACTOR = 0.5,						-- AI port strike mission factor
